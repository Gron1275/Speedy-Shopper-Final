...
class OtherFrame(wx.Frame):
    ...
    def __init__(self, title, parent=None):
        ...
        for key in requested_aisles_dict:
            ...
            globals()['self.%s' % key] = wx.StaticText(self, wx.ID_ANY, u"{}".format(str(key).title() + ':'), wx.Point(default_pos_x, y_determinate * 10), wx.DefaultSize, 0)
            globals()['self.%s' % str(key) + " items"] = wx.StaticText(self, wx.ID_ANY, u"{}".format(str(', '.join(requested_aisles_dict[key]).title())), wx.Point(default_pos_x, y_determinate * 10), wx.DefaultSize, 0)
